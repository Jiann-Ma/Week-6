...
def signup():
    mycursor.execute(f"SELECT username FROM user Where username ='{request.values['username']}'")  #先執行SQL命令，去看request.values['username']有沒有已經在DB
...
def signin():
    ...
    if request.method == "POST":
        ...
        myresult = mycursor.fetchone()  #使用fetchone就不用使用fetchall，還要使用for 迴圈
        if myresult and myresult[2]==request.form["username"] and myresult[3]==request.form["password"]: #先確認myresult有沒有存在；再確認username跟password正不正確。
